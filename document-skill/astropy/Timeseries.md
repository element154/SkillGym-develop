## Source: https://docs.astropy.org/en/stable/timeseries/index.html
...
The time series classes presented below are `QTable`

subclasses that have
special columns to represent times using the `Time`

class. Therefore, much of
...
be interested in the `BinnedTimeSeries`

subclass which we show in
Using timeseries).
...
We can then use the `TimeSeries`

class to read in this file:
...
Time series are specialized kinds of `Table`
...
In the same way as for `Table`

objects, the various columns and rows of
`TimeSeries`

objects can be accessed and sliced using index notation:
...
As seen in the previous examples, `TimeSeries`
...
The first column is always a `Time`

object (see Times and Dates), which therefore supports the ability to convert to different
...
To learn more about the capabilities in the `astropy.timeseries`
...
The details of using `astropy.timeseries`
