## Source: https://docs.astropy.org/en/stable/io/overview.html
...
## Source: https://docs.astropy.org/en/stable/io/unified.html

# High-level Unified File I/O#

`astropy`

uses the I/O Registry to provide a unified interface
...
has class methods `read()`

and `write()`

that can be used to read and write data.

The first positional argument to these methods specifies the input or output. In general the input can be a file name, a file-like object or a URL. For some formats, most notably the astropy.io.ascii formats, the input can also be a string or list of strings representing the data. The output can be a file name or a file-like object.

The file format is specified using the `format`

keyword argument. This is required
...
format automatically from the file name, so the `format`
...
The FITS writer will issue a few warnings because the units read from the DAOphot file do not match the FITS conventions, but the data in the file is perfectly fine.

Each file format is handled by a specific reader or writer, and each of those functions will have its own set of arguments.
...
class method, e.g., `astropy.table.Table.read`
...
## Source: https://docs.astropy.org/en/stable/io/fits/index.html
...
recommended method is via the High-level Unified File I/O interface. In particular
see the Unified I/O FITS section. Likewise, for CCD image
data with a physical unit (e.g., `electron`

), see the Unified Image Data section.
...
which take either the relative or absolute path as string or path-like object.
...
There are some limitations when working with compressed files. For example, with Zip files that contain multiple compressed files, only the first file will be accessible. Also bzip2 and lzma do not support the append or update access mode and LZW-compressed files do not support any writing modes (including append or update).
...
package directly. For CCD image data with a unit, you should
consider using the Unified Image Data interface with the
CCDData class. This provides the the capability to load data,
...
package directly. If you want to read or write a single
entire table in FITS format, the you should consider using the High-level Unified File I/O
interface (e.g., `QTable.read`

). See the Unified I/O FITS
section for details.
...
## Source: https://docs.astropy.org/en/stable/io/ascii/index.html
...
text data table formats via built-in Extension Reader Classes. The
...
It is strongly encouraged to use the Unified I/O Text Tables interface rather than using `astropy.io.ascii`

directly.

For reading large CSV files, the astropy PyArrow CSV reader is the fastest option, while for writing large data tables to CSV, the Table - Pandas interface is an option to consider.

Additional information is available in the Unified I/O and Unified I/O Table Data pages.

The following shows a few of the text formats that are available, while the section on Supported formats contains the full list.
...
: table from the IRAF DAOphot package`Ecsv`
...
: LaTeX table with datavalue in the`tabular`
...
by trying most of the supported formats.
...
function provides a way to write a data table as a formatted text
...
for text tables is given below. The `Write`
...
documentation for the specific format class (e.g. `HTML`
...
## Source: https://docs.astropy.org/en/stable/io/votable/index.html
...
sub-package converts VOTable XML files to and
from `numpy`
...
package directly. For some cases, however, the high-level
...
- Attempt to parse the VOTable silently. This is the default setting.

`'warn'`

- Attempt to parse the VOTable, but raise appropriate Warnings. It is possible to limit the number of warnings of the same type to a maximum value using the`astropy.io.votable.exceptions.conf.max_warnings`

item in the Configuration System (astropy.config).

`'exception'`

- Do not parse the VOTable and raise an exception.

The `verify`

keyword can be used with the `parse()`

or `parse_single_table()`
...
It is possible to change the default `verify`

value through the
`astropy.io.votable.conf.verify`
...
writer, since it requires writing multiple files.`PARQUET`

stores the data in an external PARQUET file, similar to FITS serialization. Reading and writing is fully supported by the`astropy.io.votable`

writer and the`astropy.io.votable.parse`

reader. The parquet file can be referenced with either absolute and relative paths. The parquet serialization can be used as part of the unified Table I/O (see next section), by setting the`format`
...
attribute of a`astropy.io.votable.tree.TableElement`
...
but column names are required to be unique in `numpy`
...
File reads will be moderately faster if the `TABLE`
...
## Source: https://docs.astropy.org/en/stable/io/misc.html
...
class itself (see
High-level Unified File I/O). Routines that are intended to be used directly by users are
listed in the `astropy.io.misc`

section.

## Source: https://docs.astropy.org/en/stable/io/registry.html
...
about what built-in formats are supported by `Table`

by default should see High-level Unified File I/O.
Likewise Cosmology I/O & Convert for built-in formats supported by
`Cosmology`
...
, but this will be added in future.
...
written as a function that can take any arguments except `format`
...
In practice, it would be nice to have the `read`

method automatically
identify that this file is in the `my-table-format`
...
which indicates whether the identifier is being called from `read`

or
`write`

, and should then accept an arbitrary number of positional and keyword
arguments via `*args`

and `**kwargs`

, which are the arguments passed to
the `read`
...
files with filenames ending in `.mtf`

as being in the `my-table-format`
...
arguments — although as for the reader, the `format`

keyword argument cannot
be used.
...
–
write-only – `UnifiedOutputRegistry`

–
and read/write – `UnifiedIORegistry`
...
To make read-only or write-only registries.

To allow for different readers for the same format.

To allow for an object to have different

kindsof readers and writers. E.g.`Cosmology`

which supports both file I/O and object conversion.

## Source: https://docs.astropy.org/en/stable/io/typing.html
...
provides type annotations through the `astropy.io.typing`
...
parameter can be any type of path-like object (e.g. a string, byte-string,
...
The `astropy.io.typing`

module also provides type aliases for file-like objects
that support reading and writing. The following example uses the
`ReadableFileLike`

type alias to specify that the `fileobj`

parameter can be any file-like object that supports reading. Using a
`TypeVar`

, the return type of the function is specified to be the same
...
objects can also be used as runtime-checkable `Protocol`
...
|
A file-like object that supports reading with a method |
|
A file-like object that supports writing with a method |
