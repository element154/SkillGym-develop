...
of lines are formatted identically. (See: *Style Guide*.)
...
Black in a few conscious ways (see: *Known deviations*). In general,
...
directives. As with Markdown, the language names recognized for Python are`python`
...
*This feature is currently only available in preview mode.*
...
the following info strings: `python`
...
will be formatted like Python type stub files:

Ruff also supports Quarto style executable code blocks with curly braces surrounding the language name:

While formatting suppression comments will be handled as usual within code blocks, the formatter will also skip formatting any code block surrounded by appropriate HTML comments, such as:

Any number of code blocks may be contained within a matching pair of `off`

and
`on`

HTML comments, and any `off`

comment *without* a matching `on`
...
To format Markdown files with extensions other than `.md`

, configure custom
`extension`

mappings. Ruff will automatically include
these mapped extensions in file discovery:

If you run Ruff via `ruff-pre-commit`
...
comments suppress formatting for a case header, decorator,
...
if Ruff terminates successfully, regardless of whether any files were formatted.`1`

if Ruff terminates successfully, one or more files were formatted, and`--exit-non-zero-on-format`
...
Similar to Black, Ruff implements formatting changes
under the `preview`
...
. This is a known deviation from Black.
...
) with a format specifier containing the configured quote style:
...
.
Assuming the assigned expression exceeds the `line-length`
