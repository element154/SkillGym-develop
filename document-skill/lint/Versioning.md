...
- This
*may*occur in minor version changes until`1.0.0`
...
- Bugs are fixed,
*including behavior changes that fix bugs* - A new configuration option is added in a backwards compatible way (no formatting changes or new lint errors)
...
The minimum supported Rust version required to compile Ruff is listed in the `rust-version`
...
. It may change in any release (minor or patch). It
...
then a rule will not be eligible for stability until`0.8.0`
...
**Display**: Never applied, just displayed.**Unsafe**: Can be applied with explicit opt-in.**Safe**: Can be applied automatically.
