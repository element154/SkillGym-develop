...
workspace, whether it's a `pyproject.toml`
...
The VS Code extension provides the `Ruff: Format Document`
...
The Ruff VS Code extension provides the `Format Document`
...
comment. - Apply all quick fixes available in the document.
...
Application of unsafe fixes when using "Fix all" can be enabled by setting `unsafe-fixes = true`
...
lint or format `.ipynb`

files by default as of version 0.6.0. Refer to the
Jupyter Notebook discovery section for
more details.
