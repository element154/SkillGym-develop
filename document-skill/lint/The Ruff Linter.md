...
over`lint.extend-select`

to make your rule set explicit. - Use
`ALL`

with discretion. Enabling`ALL`

will implicitly enable new rules whenever you upgrade. - Start with a small set of rules (
`select = ["E", "F"]`

) and add a category at-a-time. For example, you might consider expanding to`select = ["E", "F", "B"]`
...
whether a rule supports fixing, see *Rules*.
...
To omit a lint rule everywhere, add it to the "ignore" list via the `lint.ignore`
...
comment is given by a case-insensitive match for`#noqa`

with optional whitespace after the`#`

symbol, followed by either: the end of the comment, the beginning of a new comment (`#`

), or whitespace followed by any character other than`:`
...
symbol, optional whitespace after`noqa`

, and followed by the symbol`:`

. After this we are expected to have a list of rule codes which is given by sequences of uppercase ASCII characters followed by ASCII digits, separated by whitespace or commas. The list ends at the last valid code. We will attempt to interpret rules with a missing delimiter (e.g.`F401F841`
...
For this reason, a `RUF104`

diagnostic will also be produced for any implicit range.
If implicit range suppressions are desired, the `RUF104`
...
suppressions, range suppressions do not support "blanket" suppression
...
- An own-line comment starting with case sensitive
`#ruff:`
...
symbol, followed by either`disable`

or`enable`

to start or end a range respectively, immediately followed by`[`
...
- A file-level exemption comment is given by a case-sensitive match for
`#ruff:`
...
, followed by optional whitespace and a case-insensitive match for`noqa`

. After this, the specification is as in the inline`noqa`
...
code, to enforce that your suppressions are "valid", in that the violations
they *say* they ignore are actually being triggered and suppressed. To flag
unused suppression comments, run Ruff with `--extend-select RUF100`

, like so:

Ruff can also *remove* any unused suppression comments via its fix functionality.
To remove any unused suppressions, run Ruff with `--fix`
...
directives to all
relevant lines (with the appropriate rule codes), run Ruff with `--add-noqa`
...
will cause Ruff to exit with a status code of`0`

even if violations were found. Note that Ruff will still exit with a status code of`2`

if it terminates abnormally.`--exit-non-zero-on-fix`

will cause Ruff to exit with a status code of`1`
