...
file for a given path, Ruff ignores any`pyproject.toml`
...
, those settings are used for*all*analyzed files, and any relative paths in that configuration file (like`exclude`
...
being determined via`etcetera`

's base strategy, and all relative paths being again resolved relative to the*current working directory*. - Any config-file-supported settings that are provided on the command-line (e.g., via
`--select`
...
When no discovered configuration specifies a `target-version`

, Ruff will attempt to fall back to the minimum version compatible with the `requires-python`

field in a nearby `pyproject.toml`

.
The rules for this behavior are as follows:

- If a configuration file is passed directly, Ruff does not attempt to infer a missing
`target-version`

. - If a configuration file is found in the filesystem hierarchy, Ruff will infer a missing
`target-version`

from the`requires-python`

field in a`pyproject.toml`

file in the same directory as the found configuration. - If we are using a user-level configuration from
`${config_dir}/ruff/pyproject.toml`

, the`requires-python`

field in the first`pyproject.toml`

file found in an ancestor of the current working directory takes precedence over the`target-version`

in the user-level configuration. - If no configuration files are found, Ruff will infer the
`target-version`
...
file found in an ancestor of the current working directory.
...
is also enabled (via CLI or settings file).
For example, without `force-exclude`

enabled, `ruff check /path/to/excluded/file.py`
...
.
In preview mode, Ruff will also discover `*.pyw`
