...
SymPy runs under the [Python Programming Language](https://www.python.org/),
...
of the gotchas and pitfalls that you may encounter when using SymPy. See also the [introductory
tutorial](https://docs.sympy.org/latest/tutorials/intro-tutorial/index.html#intro-tutorial),
the remainder of the SymPy Docs, and the [official
Python Tutorial](https://docs.python.org/3/tutorial/).

If you are already familiar with C or Java, you might also want to look
at this [4 minute Python tutorial](https://nerdparadise.com/programming/python4minutes/).
...
you want to do x = y, use `Eq(x, y)` for equality.
...
expand(), simplify(), and trigsimp() and see if the
...
>>> z**2
z**2
If you use isympy, it runs the following commands for you,
...
Symbol and symbols(). See Keyword
Arguments below.

Lastly, it is recommended that you not use I, E, S, N, C, O, or Q for variable or symbol names, as those
are used for the imaginary unit (i), the base of the natural logarithm
(e), the sympify() function (see Symbolic
Expressions below), numeric evaluation (N() is
equivalent to evalf() ), the big O order symbol (as in
O(n log n)), and the assumptions object that holds a list of
...
If you have IPython installed and
use isympy, you can also press the TAB key to get a list of
all built-in names and to autocomplete. Also, see this page for a
trick for getting tab completion in the regular Python console.

Note

See also the Defining Symbols section of the
Best Practices page.
...
to a Python expression. Use the sympify() function, or just
S, to ensure that something is a SymPy expression.
...
to them. The solution is to sympify() one of the numbers, or use
Rational (or Python's Fraction).
...
reusing it. If the expression has a Rational (i.e.,
...
>>> S("22/7")
22/7
Rational only works for number/number and is only meant for
...
Pass the desired substitutions with the call to evaluate. By doing
...
-0.e-91
Use Rationals, not Floats. During the evaluation process, the
...
0.e-91
Try to simplify the expression. In this case, SymPy will recognize
...
if __name__ == "__main__":
    main()
The output shows that the subs() function has replaced variable
x with variable a, and variable y with variable b:

text
original = 3*x + 4*y
modified = 3*a + 4*b
The subs() function does not modify the original expression expr.
...
Python numbers vs. SymPy Numbers above,
...
of asin, acos and so on instead of the usual arcsin
and arccos. Use the methods described in Symbols
...
tuple is identical to a list, except that it is not
...
Keyword Arguments
Aside from the usage described above, equals signs
...
help()
Although all docs are available at docs.sympy.org or on the
SymPy Wiki, you can also get info on functions from within the
Python interpreter that runs SymPy. The easiest way to do this is to do
help(function), or function? if you are using ipython:
...
powsimp(). The output will look something like this:
