...
- **Define symbols with [`symbols()`](https://docs.sympy.org/latest/modules/core.html#sympy.core.symbol.symbols) or [`Symbol()`](https://docs.sympy.org/latest/modules/core.html#sympy.core.symbol.Symbol).** The
  ...
  and defining [`Function`](https://docs.sympy.org/latest/modules/core.html#sympy.core.function.Function) objects:
  ...
  The [`var()`](https://docs.sympy.org/latest/modules/core.html#sympy.core.symbol.var) function should be avoided, except when working
  interactively. It works like the `symbols()` function, except it
  ...
  [Avoid String Inputs](#avoid-string-inputs) section below apply here.

- **Add assumptions to symbols when they are known.**
  [Assumptions](https://docs.sympy.org/latest/guides/assumptions.html#assumptions-guide) can be added by passing the relevant
  keywords to `symbols()`. The most common assumptions are `real=True`,
  ...
  Here, ∫₀∞ e^(-ax) dx gives a piecewise result when `a`
...
See also [Avoid String Inputs](#avoid-string-inputs) and
[Don't Hardcode Symbol Names in Python Functions](#dont-hardcode-symbol-names-in-python-functions) for related best practices
...
[`parse_expr()`](https://docs.sympy.org/latest/modules/parsing.html#sympy.parsing.sympy_parser.parse_expr). It is best to parse
...
- Support for string inputs in general SymPy functions is mostly accidental.
  It happens because these functions call [`sympify()`](https://docs.sympy.org/latest/modules/core.html#sympy.core.sympify.sympify) on their inputs in
  ...
  functions (other than `sympify()` or [`parse_expr()`](https://docs.sympy.org/latest/modules/parsing.html#sympy.parsing.sympy_parser.parse_expr)) [may go away in
...
- The biggest gotcha when using string inputs comes from using assumptions. In
  ...
  expressions like the one above (see [Defining Symbols](#defining-symbols)
  above).
  ...
  dictionary to [`parse_expr()`](https://docs.sympy.org/latest/modules/parsing.html#sympy.parsing.sympy_parser.parse_expr). For example:
...
- Symbol names can contain any character, including things that aren't
  ...
  What happened here is that instead of parsing `x^1_2` as x₁², it is
...
numbers](https://docs.sympy.org/latest/modules/core.html#sympy.core.numbers.Float), but some operations may not perform as
well with them.
...
former is a numerical approximation to π and the latter is exactly π
(see also [Separate Symbolic and Numeric Code](#separate-symbolic-and-numeric-code) below; in
...
Here `sympy.sin(math.pi)` is not exactly 0, because `math.pi` is not exactly π.
...
In this case, use [`Rational`](https://docs.sympy.org/latest/modules/core.html#sympy.core.numbers.Rational) to create a rational number, or use
...
- An exact symbolic value can often be symbolically simplified or manipulated.
  ...
  `sin(math.pi)` does not produce `0` because `math.pi` is not exactly π.
  It is just a floating-point number that approximates π to 15 digits
  (effectively, a close rational approximation to π, but not exactly
  π).
...
[`simplify()`](https://docs.sympy.org/latest/modules/simplify/simplify.html#sympy.simplify.simplify.simplify) (not to be confused with [`sympify()`](https://docs.sympy.org/latest/modules/core.html#sympy.core.sympify.sympify)) is designed as
...
functions](https://docs.sympy.org/latest/modules/simplify/simplify.html#simplify-docs) instead (e.g., [`cancel()`](https://docs.sympy.org/latest/modules/polys/reference.html#sympy.polys.polytools.cancel), [`expand()`](https://docs.sympy.org/latest/modules/core.html#sympy.core.function.expand),
or [`collect()`](https://docs.sympy.org/latest/modules/simplify/simplify.html#sympy.simplify.radsimp.collect)).
...
- There are no guarantees about what form an expression will have after being
  ...
  - [`factor()`](https://docs.sympy.org/latest/modules/polys/reference.html#sympy.polys.polytools.factor) will always factor a polynomial into irreducible factors.
  - [`cancel()`](https://docs.sympy.org/latest/modules/polys/reference.html#sympy.polys.polytools.cancel) will always convert a rational function into the form
...
be taken with them as well. For example, the [`trigsimp()`](https://docs.sympy.org/latest/modules/simplify/simplify.html#sympy.simplify.trigsimp.trigsimp) function is a
heuristic targeted to trigonometric functions, but the routines in the
[`sympy.simplify.fu`](https://docs.sympy.org/latest/modules/simplify/fu.html#module-sympy.simplify.fu) submodule allow applying specific trigonometric
identities.

The [simplify section of the tutorial](https://docs.sympy.org/latest/tutorials/intro-tutorial/simplification.html#tutorial-simplify) and the
[simplify module reference](https://docs.sympy.org/latest/modules/simplify/simplify.html) list the various
...
simplification using [`replace()`](https://docs.sympy.org/latest/modules/core.html#sympy.core.basic.Basic.replace), or in general, manually using
[advanced expression manipulation](https://docs.sympy.org/latest/tutorials/intro-tutorial/manipulation.html#tutorial-manipulation).

### Don't Hardcode Symbol Names in Python Functions

Instead of hard-coding [`Symbol`](https://docs.sympy.org/latest/modules/core.html#sympy.core.symbol.Symbol) names inside of a function
...
operator](https://en.wikipedia.org/wiki/Theta_operator) θ = z D_z:
...
compute θ = x D_x because it is hard-coded to z D_z. What's worse,
...
[Defining Symbols](#defining-symbols) above), the function hard-coding
...
By making the symbol an argument to the function, like `theta_operator(expr, z)`, these problems all go away.
...
SymPy expressions into NumPy functions using [`lambdify()`](https://docs.sympy.org/latest/modules/utilities/lambdify.html#sympy.utilities.lambdify.lambdify). The typical
...
generation](https://docs.sympy.org/latest/modules/codegen.html#codegen-prose) routines to generate code for other fast numerical
...
- **Do not use `import math`.** It is virtually never necessary to use the
  standard library [`math`](https://docs.python.org/3/library/math.html)
  module alongside SymPy (or
  NumPy). Every function that is in `math` is already in SymPy. SymPy can
  compute values numerically using [`evalf`](https://docs.sympy.org/latest/explanation/glossary.html#term-evalf), which provides more
  ...
  `math.pi` is only an approximation of π, equal to 16 digits. On the
  other hand, `sympy.pi` is *exactly* equal to π because it is represented
  ...
  representations](#exact-rational-numbers-vs-floats). But even
...
- **Don't pass SymPy expressions to a lambdified function.** Similar to the
  ...
  it works" section of the `lambdify()` documentation](https://docs.sympy.org/latest/modules/utilities/lambdify.html#lambdify-how-it-works)
...
- **Avoid storing SymPy expressions in a NumPy array.** While it is
  ...
  [`Float`s](https://docs.sympy.org/latest/modules/core.html#sympy.core.numbers.Float).
  ...
  you should use SymPy's [`Matrix`](https://docs.sympy.org/latest/modules/matrices/dense.html#sympy.matrices.dense.Matrix) or
  `NDimArray` classes.
...
ever be symbolic, the above code is wrong. It will fail with `TypeError: cannot determine truth value of Relational`. If you ever see this exception,
...
- **Disallow symbolic inputs.** If your function cannot possibly work on
  ...
  is to give a more readable error message to users than `TypeError:  cannot determine truth value of Relational`. The
  [`is_number`](https://docs.sympy.org/latest/modules/core.html#sympy.core.expr.Expr.is_number) attribute can be used to check if an
...
- **Use the assumptions system.** If you do support symbolic inputs, you
  ...
  nuances](https://docs.sympy.org/latest/guides/booleans.html#booleans-guide) of the [three-valued fuzzy logic](https://docs.sympy.org/latest/explanation/glossary.html#term-Three-valued-logic) used in the assumptions system. That is, always be
...
- **Return a Piecewise result.** If the result of a function depends on an
  inequality or other boolean condition, you can use [`Piecewise`](https://docs.sympy.org/latest/modules/functions/elementary.html#sympy.functions.elementary.piecewise.Piecewise) to
...
- **Use [`ordered()`](https://docs.sympy.org/latest/modules/core.html#sympy.core.sorting.ordered) to sort expressions into a canonical order.**
  If you are trying to use `sorted` because you want a canonical ordering, but
  you don't particularly care what that ordering is, you can use `ordered`.
...
[`Basic`](https://docs.sympy.org/latest/explanation/glossary.html#term-Basic), [`Expr`](https://docs.sympy.org/latest/explanation/glossary.html#term-Expr), or [`Function`](https://docs.sympy.org/latest/explanation/glossary.html#term-Function-class). All the
...
writing custom functions](https://docs.sympy.org/latest/guides/custom-functions.html#custom-functions).
...
The first says that all elements of [`args`](https://docs.sympy.org/latest/explanation/glossary.html#term-args) should be instances of
[`Basic`](https://docs.sympy.org/latest/explanation/glossary.html#term-Basic). The second says that an expression should be rebuildable from
its `args` (note that [`func`](https://docs.sympy.org/latest/explanation/glossary.html#term-func) is usually the same as `type(expr)`, though
...
2. If an expression did not rebuild from its `args`, the line `return expr.func(*newargs)` would fail, even in the trivial case where none of the
...
`Function` (see the [guide to writing custom functions](https://docs.sympy.org/latest/guides/custom-functions.html#custom-functions)).
...
guide](https://docs.sympy.org/latest/guides/custom-functions.html#custom-functions) for a list of common simplification routines that can
be defined on SymPy objects).

The [custom functions guide](https://docs.sympy.org/latest/guides/custom-functions.html#custom-functions-automatic-evaluation) goes over
...
For example, take the [`FiniteSet`](https://docs.sympy.org/latest/modules/sets.html#sympy.sets.sets.FiniteSet) class. It is constructed like
...
makes things easier to deal with using the object's [`args`](https://docs.sympy.org/latest/explanation/glossary.html#term-args), since
...
addition to finite ones, like [`Integers`](https://docs.sympy.org/latest/modules/sets.html#sympy.sets.fancysets.Integers) or [`Range`](https://docs.sympy.org/latest/modules/sets.html#sympy.sets.fancysets.Range), then it
...
in their [`args`](https://docs.sympy.org/latest/explanation/glossary.html#term-args). For instance, this breaks `==` checking, which only
compares an objects `args`. See the [Don't Overwrite `__eq__`](#dont-overwrite-eq) section below for
...
- **Store the extra data in the object's `args`.** This is the best approach if
  ...
  the object](#args-invariants) (e.g., something like
  ...
  [Writing Custom Functions](https://docs.sympy.org/latest/guides/custom-functions.html#custom-functions)) or `Expr` directly. If you simply want to have two
  ...
  to print things, or use a [custom printer](https://docs.sympy.org/latest/modules/printing.html#module-printing).
...
- **Store the data about the object separately.** This is the best approach if
  ...
  dictionary keys. So maintaining a separate dictionary of `{object: extra_data}` pairs is straightforward.
  ...
  [printers](https://docs.sympy.org/latest/explanation/glossary.html#term-Printing), which allow defining [custom
  printers](https://docs.sympy.org/latest/modules/printing.html#module-printing) that change how any SymPy object is printed
  without modifying those object themselves. Functions like [`lambdify()`](https://docs.sympy.org/latest/modules/utilities/lambdify.html#sympy.utilities.lambdify.lambdify)
  and [`init_printing()`](https://docs.sympy.org/latest/modules/interactive.html#sympy.interactive.printing.init_printing) allow passing in a custom printer.
...
- **If the data you want to store is a Python function**, it's best to just
  ...
  one of the [existing set of overridable SymPy methods](https://docs.sympy.org/latest/guides/custom-functions.html#custom-functions). If
  you want to define how a function evaluates itself numerically, you can use
  [`implemented_function()`](https://docs.sympy.org/latest/modules/utilities/lambdify.html#sympy.utilities.lambdify.implemented_function).

- **Represent the information using by modifying the object's `func`.**
  This solution is much more complicated than the others, and should only be
  used when it is necessary. In some extreme cases, it is not possible to
  represent every mathematical aspect of an object using `args` alone. This
  can happen, for example, because of the limitation that [`args` should only contain
  `Basic` instances](#args-invariants). It is still possible to
  create custom SymPy objects in these situations by using a custom
  [`func`](https://docs.sympy.org/latest/explanation/glossary.html#term-func) that is different from `type(expr)` (in this case, you would
  override `__eq__` on the `func` [rather than on the
  class](#dont-overwrite-eq)).
...
In SymPy, `==` compares objects using [structural equality](https://docs.sympy.org/latest/explanation/glossary.html#term-Structural-Equality). That is, `a == b` means that `a` and `b` are exactly the same object. They have the same
type and the same [`args`](https://docs.sympy.org/latest/explanation/glossary.html#term-args). `==` does not perform any sort of
...
be represented with [`Eq`](https://docs.sympy.org/latest/modules/core.html#sympy.core.relational.Equality).
...
SymPy objects are [immutable](https://docs.sympy.org/latest/explanation/glossary.html#term-Immutable). When `a == `, any SymPy function may
freely replace `a` with `b` in any subexpression.

The default `__eq__` method on [`Basic`](https://docs.sympy.org/latest/explanation/glossary.html#term-Basic) checks if the two objects have
...
it (see [`args`](https://docs.sympy.org/latest/explanation/glossary.html#term-args)). Note that it is possible for an objects constructor to
...
- To make `==` apply some smarter equality check than purely structural
  ...
  Another option is to define a [canonicalization](https://docs.sympy.org/latest/explanation/glossary.html#term-Canonicalize) method
...
- To make `==` check for some additional attributes beyond those stored in the
  `args` of an expression. See the [Avoid Storing Extra Attributes on an Object](#avoid-storing-extra-attributes-on-an-object) section
...
- To make `==` compare equal to some non-SymPy object. It is preferable to
  ...
  extending the `converter` dictionary. See the [`sympify()`](https://docs.sympy.org/latest/modules/core.html#sympy.core.sympify.sympify)
...
(see the [custom
functions guide](https://docs.sympy.org/latest/guides/custom-functions.html#custom-functions-assumptions) for details on how to do this),
...
This generally means using methods like [`as_independent()`](https://docs.sympy.org/latest/modules/core.html#sympy.core.expr.Expr.as_independent) or
[`as_coeff_mul()`](https://docs.sympy.org/latest/modules/core.html#sympy.core.expr.Expr.as_coeff_mul) and checking
the `args` of expressions directly (see the [custom functions
guide](https://docs.sympy.org/latest/guides/custom-functions.html#custom-functions-assumptions) for an example).
...
<p>Python dicts and sets use `hash`, but fallback to using `==`
